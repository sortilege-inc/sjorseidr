...
| [`covenant_ledger.html`](covenant_ledger.html) | `covenant_ledger.json` | **Canonical covenant page.** A tabbed hub: **Covenant** (the former covenant sheet — founding year, tribunal, saga, aegis/regio, loyalty, finances, covenant-wide virtues & flaws, Charter and founding note), Overview, Ships & Land, Labs (full lab sheets), Inhabitants (seasonal crew breakdown), Library, Vis, Trade, Economics, and Ledger. The former `covenant.html` was folded into the **Covenant** tab and has been **removed** — use `covenant_ledger.html#covenant` (old `covenant.html` links no longer resolve). `ships.html`'s Fleet & Crew was folded into **Ships & Land** / **Inhabitants** and now survives only as a redirect stub pointing here. |
